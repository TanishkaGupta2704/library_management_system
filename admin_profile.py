...
class Profile_sub:
    def __init__(self, root):
            ...
            def update():
                ...
                connection = sqlite3.connect("admin_database.db")
                ...
                def save():
                    connection = sqlite3.connect("admin_database.db")
                    cursor = connection.cursor()
                    cursor.execute("DELETE FROM admins1")


                    # Execute SQL command to delete records
                    # cursor.execute("DELETE FROM admins")
                    cursor.execute("INSERT INTO admins1 VALUES (:var_name, :var_adminID)",
                      {
                          'var_name': entry20.get(),
                          'var_adminID': entry21.get()
            ...
            c=conn.cursor()
            c.execute("SELECT name FROM admins1")
            ...
            c.execute("SELECT adminID from admins1")
